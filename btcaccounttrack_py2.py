...
element1 = soup1.select(
    '[class="sc-8sty72-0 bFeqhe"] [class="sc-1ryi78w-0 cILyoi sc-16b9dsl-1 ZwupP u3ufsr-0 eQTRKC"]')[2]
print 'WannaCry Account_01 = ', element1.text
element2 = soup2.select(
    '[class="sc-8sty72-0 bFeqhe"] [class="sc-1ryi78w-0 cILyoi sc-16b9dsl-1 ZwupP u3ufsr-0 eQTRKC"]')[2]
print 'WannaCry Account_02 = ', element2.text
element3 = soup3.select(
    '[class="sc-8sty72-0 bFeqhe"] [class="sc-1ryi78w-0 cILyoi sc-16b9dsl-1 ZwupP u3ufsr-0 eQTRKC"]')[2]
print 'WannaCry Account_03 = ', element3.text

total = float(element1.text.replace(' BTC', '')) + \
    float(element2.text.replace(' BTC', '')) + \
    float(element3.text.replace(' BTC', ''))
print 'WannaCry total received =', total, 'BTC'
print 'WannaCry total received =', total * \
    1750, 'USD', '(in the rate 1BTC=1750 USD)'
print 'WannaCry total received =', total*1750 * \
    30.2, 'USD', '(in the rate 1USD=30.2 TWD)'
